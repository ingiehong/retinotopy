...
% Last Modified by GUIDE v2.5 15-Dec-2005 12:19:22
...
scl = serial('COM6','Tag','clser','Terminator','CR',...
...
%%%illuminator serial 
sit = serial('COM4','Tag','ilser','Terminator','CR','DataTerminalReady',...
    'off','RequestToSend','off');
fopen(sit);
handles.sit = sit;
...
% %% the z-axis motor...
% sza = serial('COM21','Tag','zaser','Terminator','CR','DataTerminalReady',...
%     'off','RequestToSend','off','Baud',38400);
% fopen(sza);
% handles.sza = sza;
% fwrite(sza,['4 EN' 13 10]);  %% Enable
% 
% 
% spr = serial('COM1','Tag','prser','Terminator','CR/LF',...
%     'InputBufferSize',2048);  %% Photo research 650
% fopen(spr);
% handles.spr = spr;
...
%%%% MY code...
handles.mildig.set('Format','C:\imager\2x2bin_dlr.dcf');  %Set the binning to 2x2
y = clsend('sbm 2 2');
...
n = get(imagerhandles.masterlink,'BytesAvailable');
if n > 0
    cmd = char(fread(imagerhandles.masterlink,n));
else
    return
end
cmd = cmd';
%cmd = cmd(1:end-2)'  %Get rid of the terminator
...
switch(cmd(1))
    ...
    case 'S'  %% start sampling...
        ...
        global FPS IMGSIZE nframes maxframes ...
        ...
        dd = [datadir '\' lower(animal) '\u' unit '\' expt '\']
        ...
        if(proceed)
            ...
            %%  T = zeros(1,maxframes);  %% times now collected by cerebus
            nframes = 1;
            ...
            %% Stream to disk using double buffering
            ...
            %% This timer line
            %% T(n)=invoke(h.milapp.Timer,'Read');
            %% was removed from after putvalue=1 as the cerebus is now sampling
            %% the pulses instead.

            %% Use the following to read an image:
            %%img = h.milimg.Get(zz,262144,-1,0,0,IMGSIZE,IMGSIZE);

            %%playsound('samplingimages.wav');               
            
            %% playblocking(h.blip);  %% an audio blip that serves as a sync pulse...
            %% play(h.blip)
            %%  
            %%h.child{2-bitand(n,1)}.Save([fname '_' sprintf('%08d',n-1) '.raw']);   %% Save the child!!!!
...
%             end
            ...
            fname = [fname  '_' sprintf('%04d',timetag)]
            running = 1;
            h = GrabSaveLoop(h,fname,parport,Nim);

            set(1,'Name','imager');
            drawnow;
            running = 0;
            %%playsound('samplingstopped.wav');
        end

    case '@'
        global running;
        running = 0;  %% end of stimulus...  stop!
end
...
smarthome('motor',get(hObject,'Value'));
